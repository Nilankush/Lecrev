...
export const projectCreateController = async (req, res) => {
    ...
    const project_id = projectId ? (projectId.toLowerCase() + "-" + generateSlug(2)) : generateSlug(2);
    if (!gitUrl || !project_id || !buildScript)
...
export const deleteProjectController = async (req, res) => {
    try {
        ...
        const folderPath = req.params.prefix;
...
async function deleteFolder(id, prefix) {
    if (!prefix.endsWith("/"))
        prefix += "/";
    let continuationToken;
    let totalDeleted = 0;
    do {
        const listResponse = await R2_client.send(new ListObjectsV2Command({
        ...
        if (!contents || contents.length === 0)
            break;
        const deleteResponse = await R2_client.send(new DeleteObjectsCommand({
            Bucket: process.env.BUCKET_NAME,
            Delete: {
...
//# sourceMappingURL=projectRouterController.js.map